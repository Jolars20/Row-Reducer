...
void printMatrice(vector<vector<float>> matrice, string title);
...
int main()
{
    ...
    do
    {
        ...
        printMatrice(matrice, "Startmatricen er" );
        vector<vector<float>> reducedMatrice = rowReduceMatrice(matrice);
        printMatrice(reducedMatrice, "Slutmatricen er");
        cin >> c;
    } while (c != 'S');
}
vector<vector<float> > inputMatrice(int antalrækker, int antalsøjler)
...
void printMatrice(vector<vector<float>> matrice, string title )
{
    ...
    cout << title <<" Matricen ser ud sådan her: \n";
    for (int i = 0; i < antalrækker; i++)
    {
        for (int j = 0; j < antalsøjler; j++)
        {
...
vector<vector<float> > rowReduceMatrice(vector < vector<float>> inputMatrice)
{
    ...
    {
        std::string hStr = to_string(h);
        std::string rækkeStr = to_string(antalrækker);
        printMatrice(outputMatrice, "Udregner for række " + hStr + " af " + rækkeStr); // skal ændres
    ...
    for (int i = antalrækker - 1; i >= 0; i--)
    {
        for (int j = 0; j < antalsøjler; j++)
        {
            ...
            if (leadingCoeffiecient != 0 && foundCoeffiecient == false) // vi forsøger at finde en ledende koefficient
            {
                ...
                for (int k = j, k < antalsøjler, k++)
                {
                    outputMatrice[i][k] = outputMatrice[i][k] / leadingCoeffiecient;
                }

                leadingCoeffiecient = outputMatrice[i][j];
